...
class PCGame(Games):
    def getName(self):
        print(self.g_name, '- game for PC, published at',self.year, 'year.')


class PS5Game(Games):
    def getName(self):
        print(self.g_name, '- game for Play Station 5, published at',self.year, 'year.')


class XBoxGame(Games):
    def getName(self):
        print(self.g_name, '- game for Microsoft XBox, published at',self.year, 'year.')

class AdnroidGame(Games):
    def getName(self):
        print(self.g_name, '- game for Android platform, published at',self.year, 'year.')


pcgame = PCGame('DayZ', 2018)
pcgame.getName()

ps5game = PS5Game('Horizon Запретный Запад', 2022)
ps5game.getName()

xboxgame = XBoxGame('Halo Infinite', 2021)
xboxgame.getName()

anrd = AdnroidGame('Angry Birds', 2009)
anrd.getName()
